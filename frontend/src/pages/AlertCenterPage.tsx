...
function AlertCard({ alert, onMarkRead, onDismiss }: {
  ...
  return (
    <div
      className={`rounded-xl border p-4 transition-all duration-200 ${cfg.border} ${alert.is_read ? 'opacity-70' : ''} ${!alert.is_read ? cfg.bg : 'bg-white/2'}`}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {!alert.is_read && (
                <span className="h-2 w-2 rounded-full bg-blue-400 flex-shrink-0 animate-pulse" />
              )}
            </div>
            <div className="flex items-center gap-1 flex-shrink-0">
              {alert.action_url && (
                <button
                  ...
                  className="text-xs px-2 py-1 rounded-lg bg-white/10 hover:bg-white/15 text-slate-300 flex items-center gap-1 transition-colors"
                >
